...
from ydtpack.ext import Timestamp
...
def test_timestamp():
    ...
    packed = ydtpack.packb(ts)
    assert packed == b"\xd6\xff" + ts.to_bytes()
    unpacked = ydtpack.unpackb(packed)
    assert ts == unpacked
...
def test_unpack_timestamp():
    # timestamp 32
    assert ydtpack.unpackb(b"\xd6\xff\x00\x00\x00\x00") == Timestamp(0)

    # timestamp 64
    assert ydtpack.unpackb(b"\xd7\xff" + b"\x00" * 8) == Timestamp(0)
    with pytest.raises(ValueError):
        ydtpack.unpackb(b"\xd7\xff" + b"\xff" * 8)

    # timestamp 96
    assert ydtpack.unpackb(b"\xc7\x0c\xff" + b"\x00" * 12) == Timestamp(0)
    with pytest.raises(ValueError):
        ydtpack.unpackb(b"\xc7\x0c\xff" + b"\xff" * 12) == Timestamp(0)

    # Undefined
    with pytest.raises(ValueError):
        ydtpack.unpackb(b"\xd4\xff\x00")  # fixext 1
    with pytest.raises(ValueError):
        ydtpack.unpackb(b"\xd5\xff\x00\x00")  # fixext 2
    with pytest.raises(ValueError):
        ydtpack.unpackb(b"\xc7\x00\xff")  # ext8 (len=0)
    with pytest.raises(ValueError):
        ydtpack.unpackb(b"\xc7\x03\xff\0\0\0")  # ext8 (len=3)
    with pytest.raises(ValueError):
        ydtpack.unpackb(b"\xc7\x05\xff\0\0\0\0\0")  # ext8 (len=5)
...
def test_unpack_datetime():
    t = Timestamp(42, 14)
    utc = datetime.timezone.utc
    packed = ydtpack.packb(t)
    unpacked = ydtpack.unpackb(packed, timestamp=3)
...
def test_pack_unpack_before_epoch():
    ...
    packed = ydtpack.packb(t_in, datetime=True)
    unpacked = ydtpack.unpackb(packed, timestamp=3)
...
def test_pack_datetime():
    ...
    packed = ydtpack.packb(dt, datetime=True)
    packed2 = ydtpack.packb(t)
    ...
    x = []
    packed = ydtpack.packb(dt, datetime=False, default=x.append)
    assert x
    assert x[0] == dt
    assert ydtpack.unpackb(packed) is None


def test_issue451():
    # https://github.com/ydtpack/ydtpack-python/issues/451
    utc = datetime.timezone.utc
...
def test_pack_datetime_without_tzinfo():
    ...
    dt = datetime.datetime(1970, 1, 1, 0, 0, 42, 14)
    packed = ydtpack.packb(dt, datetime=True, default=lambda x: None)
    assert packed == ydtpack.packb(None)
